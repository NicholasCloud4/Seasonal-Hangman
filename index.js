...
const words = [
  "christmas", "holiday", "winter", "snow", "gift", "tree",
  "santa", "reindeer", "elf", "rudolph"
];
let word, guessArr, guessesLeft;
...
function renderGuess() {
  guessContainer.innerHTML = guessArr.map(char => `<div class="guess-char">${char}</div>`).join("");
}
...
function checkGuess(event) {
  ...
  if (!letter || button.disabled) return;
...
function loseGame() {
  guessContainer.innerHTML = `
    <div class="message">You lose! The word was: ${word}</div>
  `;
  resetButton.style.display = "block";
